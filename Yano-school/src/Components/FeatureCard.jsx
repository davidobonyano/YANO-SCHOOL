...
function FeatureCard({ icon, title, description, image }) {
  ...
  const handleMobileTap = () => {
    if (window.innerWidth < 768) {
      setShowOverlay(!showOverlay); // Toggle overlay on mobile tap
    }
  };
  ...
  return (
    <div
      ...
      } bg-white dark:bg-[#2D2D30] text-gray-700 dark:text-gray-300 rounded-2xl shadow-lg overflow-hidden`}
    >
      {/* Background image */}
      ...
      >
        <Link
          ...
          onClick={(e) => e.stopPropagation()} // Prevent closing overlay on button tap
        >
          Explore Programs
        </Link>
      </div>
